<template>
  <div class="mt-8">
    ...
    <div class="flex items-center justify-between mb-4">
      <h2 class="text-xl font-bold text-white">Nhạc sống</h2>
      <button
    ...
    <div class="flex">
      <div
        v-for="i in state.musics"
        :key="i.id"
        class="w-1/4 flex-shrink-0 gap-4 px-2"
      ...
      <div
        v-if="state.musics.length === 0"
        class="text-gray-400 text-sm">
        Hiện chưa có sự kiện nhạc sống sắp diễn ra.
      </div>
    </div>
  </div>
</template>
...
const getMusicEvents = async () => {
  try {
    const res = await axios.get('http://127.0.0.1:8000/api/music-events-index');

    if (res.data.code === 200) {
      state.musics = res.data.data;
    } else {
...
// Chuyển trang khi click vào event
